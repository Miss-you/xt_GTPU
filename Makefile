...
obj-m = xt_GTPU.o
xt_GTPU-y := xt_GTPU_core.o xt_GTPU_tab_down.o xt_GTPU_tab_up.o xt_GTPU_est.o
...
all: xtlib
	...
	echo $$(sudo pkill -9 gw 2>/dev/null)
	sudo iptables -t mangle -F
	echo $$(sudo modprobe -r xt_GTPU 2>/dev/null)
	sudo cp $(BIN_DIR)/xt_GTPU.ko /lib/modules/$(KVERSION)/kernel/
	sudo depmod
	sudo modprobe xt_GTPU
	sudo /bin/cp ./Bin/libxt_GTPU.so /lib/xtables/
	sudo /bin/cp ./Bin/libxt_GTPU.so /usr/local/lib/xtables
